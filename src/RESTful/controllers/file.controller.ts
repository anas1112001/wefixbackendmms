...
export const uploadFile = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  // IMPORTANT: For ticket attachments, ensure referenceType is TICKET_ATTACHMENT
  const fileRecord = await File.create({
    ...
    // Reference type (new schema) - MUST be TICKET_ATTACHMENT for ticket files
    referenceType: referenceType, // TICKET_ATTACHMENT for ticket files
    ...
    entityType: referenceType === FileReferenceType.COMPANY ? 'company' : 
                ...
                referenceType === FileReferenceType.TICKET_ATTACHMENT ? 'ticket' : 'user', // Legacy enum
...
export const uploadMultipleFiles = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const fileRecord = await File.create({
      ...
      entityType: entityType, // Entity type from metadata or request (for TICKET_ATTACHMENT, this should be 'user')
      entityId: entityId, // Entity ID (ticket ID)
      createdBy: user.id, // User who uploaded (legacy)
      // Ensure referenceType is set correctly for ticket attachments
      referenceType: referenceType, // TICKET_ATTACHMENT for ticket files
    } as any) as any;
